...
class LadderAudioEngine {
  constructor() {
    ...
    this.debounceMs = 3000; // Don't repeat same sound within 3s
  }
  ...
  _canPlay(key) {
    const now = Date.now();
    if (this.lastPlayedMap[key] && now - this.lastPlayedMap[key] < this.debounceMs) {
      return false;
    }
  ...
  proximity(levelId) {
    if (!this.enabled || !this._canPlay(`prox_${levelId}`)) return;
  ...
  stall(levelId) {
    if (!this.enabled || !this._canPlay(`stall_${levelId}`)) return;
    const ctx = this._getCtx();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
  ...
  displacement() {
    if (!this.enabled || !this._canPlay('displacement')) return;
    const ctx = this._getCtx();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
  ...
  sweep(levelId) {
    if (!this.enabled || !this._canPlay(`sweep_${levelId}`)) return;
    const ctx = this._getCtx();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
  ...
  entryBell() {
    if (!this.enabled || !this._canPlay('entry')) return;
    const ctx = this._getCtx();
  ...
  invalidation() {
    if (!this.enabled || !this._canPlay('invalidation')) return;
    const ctx = this._getCtx();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
  ...
  compressionTick() {
    if (!this.enabled || !this._canPlay('compress')) return;
    this.debounceMs = 5000; // Longer debounce for compression
    const ctx = this._getCtx();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
    ...
    this.debounceMs = 3000; // Reset
  }

  // Soft gong — kill zone start
  killZoneGong() {
    if (!this.enabled || !this._canPlay('killzone')) return;
    const ctx = this._getCtx();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
